...
function post_warning(){
    ...
    if (empty($p_key)) {
        echo "<div class='updated fade' id='pubkeyerror'><p>".sprintf(__('You must <a href="%1$s">enter your Po.st publisher\'s Key</a> for it to work.'), get_bloginfo('wpurl').'/wp-admin/options-general.php?page=post.php')."</p></div>";
    }
}
...
(function ()
{
    ...
    s.async = true;
    s.src = 'http://i.po.st/share/script/post-widget.js#publisherKey={$p_key}';
    ...
    if ($add_widget){
        if ($design_custom_code_on) {
            ...
            if ($design_orientation == 'horizontal') {
                if ($display_position_horizontal == 'above'){
                    $content = post_make_widget(get_permalink(), get_the_title(), $options) . $content;
                }
                if ($display_position_horizontal == 'below'){
                    $content .= post_make_widget(get_permalink(), get_the_title(), $options);
                }
            } else {
